 ...
 <html lang="es" dir="ltr">
   <head>
     <meta charset="utf-8">
		 ...
		 $sql2 ="SELECT unidad_medida
		 from productos group by unidad_medida";
			 	...
			 	<div class="col-sm-4">
			 		<form id="formularioarticulos">
						...
						<select class="form-control input-sm" name="medida" id="medida">
							<option value="A">Seleccione unidad de medida</option>
						<?php while($medida=mysqli_fetch_row($result2)): ?>
							<option value="<?php echo $medida[0] ?>"><?php echo $medida[0]; ?>
							</option>
						<?php endwhile; ?>
						</select>
						<p></p>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
					...
					<input type="number" class="form-control input-sm" id="precioA" name="precioA">
											<label>Unidad de medida</label>
					<select class="form-control input-sm" name="medidaA" id="medidaA">
						<option value="A">Seleccione unidad de medida</option>
					<?php while($medida=mysqli_fetch_row($result2)): ?>
						<option value="<?php echo $medida[0] ?>"><?php echo $medida[0]; ?>
						</option>
		...
		function actualizardatosproducto(idarticulo){
			$.ajax({
				...
				success:function(r){
					...
					$('#medidaA').val(dato['unidad_medida']);
				}
			});
		}
		function eliminarproducto(idArticulo){
	alertify.confirm('¿Desea eliminar esta producto?', function(){
		$.ajax({
			type:"POST",
